       ...
       // Don't make a copy on Posix. Slash is a path separator, back slashes are not accepted as a substitute.
        ...
        if (load_model) {
            ...
            if ((!is_project_file) && (!load_old_project)) {
             ...
             // The msgid below is upstream's current wording verbatim - the patch
             // carried an older variant that would no longer match the catalogs.
...
#else
    wxGetApp().mainframe->SetTitle(m_project_name + " - OrcaSlicer MCP");
...
void Plater::priv::update_title_dirty_status()
{
    ...
    wxGetApp().mainframe->update_title_colour_after_set_title();
#else
    wxGetApp().mainframe->SetTitle(title + " - OrcaSlicer MCP");
